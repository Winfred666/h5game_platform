...
export default function NewGamePage({
  ...
  useEffect(() => {
    if (game?.tags) {
      setSelectedTags(game.tags.map((tag: IGameTag) => tag));
    }
    if (game?.developer)
      setDevelopers(
        game?.developer?.filter((dev: IDeveloper) => String(dev.id) !== cur_user_id)
      );
    setKind(game?.online ? "html" : "downloadable");
    if(game?.online) {
      setRunKind(
        (game?.online as IOnlineEmbed).width == null
          ? "fullscreen"
          : "embed_in_page"
      );
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!uploadFilename) formData.delete("uploadfile");
    ...
    // 发送POST请求到服务器
    try {
      const response = await fetch( `${process.env.NEXT_PUBLIC_SERVER_URL}/game?id=${gameId}`, {
        method: "PUT",
        body: formData,
      ...
      if (data.detail && !response.ok) {
        if(typeof data.detail == "string") snackBar.open(data.detail);
        throw new Error(data.detail);
      }
      if (!response.ok) throw new Error("Network response was not ok");
      // 刷新缓存，跳转到游戏详情页面
      mutate_path(`/game?id=${gameId}`);
      router.push(ALL_NAVPATH.game_id.href(gameId)); //使用useRouter()的push方法跳转到游戏详情页
    } catch (error) {
      console.error("Error:", error);
      snackBar.open("提交失败，请重试！");
      ...
      <Paper className="m-10 p-4" variant="outlined">
        ...
        <div className=" flex flex-row justify-between gap-10">
          <div className=" flex flex-col gap-2 grow">
            <div>
              <TextField
                key={game?.title || "title"} //key变化时组件会重新渲染
                id="title"
                name="title"
                label="Title"
            ...
            <Box sx={{ mt: 2 }}>
              {kind === "html" && (
                <Typography variant="body1" gutterBottom>
                  ...
                  contains your entire game.（注:不修改默认为原文件）
                </Typography>
              )}
              <input
                id="uploadfile"
                ...
                {runKind === "embed_in_page" && (
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", mt: 1 }}>
                      <TextField
                        key={(game?.online as IOnlineEmbed).width || "width"} //key变化时组件会重新渲染
                        id="width"
                        name="width"
                        placeholder="640"
                      ...
                      <TextField
                        key={(game?.online as IOnlineEmbed).height || "height"} //key变化时组件会重新渲染
                        id="height"
                        name="height"
                        placeholder="360"
            ...
            <TextField
              key={game?.description || "description"} //key变化时组件会重新渲染
              id="description"
              name="description"
              label="Description"
              placeholder=""
          ...
          <div className=" min-w-[40%]">
                ...
                {cover.url ? (
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">
                      You may need to re-upload a cover image.
                    </Typography>
                  </Box>
                )}
                <input
            ...
            <Box>
              ...
              <Typography variant="caption" display="block" gutterBottom>
                ...
                <br />
                （注:这里不会显示你之前上传的截图.你新上传的截图将直接覆盖原截图.）
              </Typography>
              <input
                type="file"
                accept="image/*"
        ...
        <Box sx={{ mt: 4 }}>
          ...
          >
            {isSubmitting ? "Submitting..." : "Submit"} {/* 按钮显示加载状态 */}
          </Button>
        </Box>
      </Paper>
    </form>
